...
def preprocess(data, embeddings):
    processed_data = [] 
    ...
    for review in data:
        ...
        for word in review.split(' '):
            if (word not in sklearn.feature_extraction._stop_words.ENGLISH_STOP_WORDS and word in embeddings):
                word_vectors.append(embeddings[word]) 
        
        word_vectors = np.array(word_vectors)
        
        processed_data.append(np.average(word_vectors, axis=0))
...
def main(args: argparse.Namespace) -> Optional[npt.ArrayLike]:
    ...
    if args.predict is None:
        ...
        train_as_vectors = preprocess(train.data, word_embeddings)
        ...
        model = MLPClassifier([200,2], 'relu', solver="adam", learning_rate='adaptive')
        # model = KNeighborsClassifier()
        # model = LogisticRegression()
        # model = LogisticRegressionCV(cv=5, solver='lbfgs', max_iter=1000)
        ...
        test_as_vectors = preprocess(test.data, word_embeddings)
