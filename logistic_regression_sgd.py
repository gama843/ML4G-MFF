...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(data, target, random_state=args.seed, test_size=args.test_size)
    ...
    for epoch in range(args.epochs):
        ...
        y_pred = np.apply_along_axis(sigmoid, 0, train_data @ weights)
        y_pred = np.apply_along_axis(np.rint, 0, y_pred)

        train_accuracy = sklearn.metrics.accuracy_score(train_target, y_pred)

        y_pred = np.apply_along_axis(sigmoid, 0, test_data @ weights)
        y_pred = np.apply_along_axis(np.rint, 0, y_pred)
        ...
        for x, y in zip(train_data, train_target):
            pred = sigmoid(x @ weights)
            item_loss = -(y * np.log(pred) + (1-y) * np.log(1-pred))
