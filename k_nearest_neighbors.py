...
def get_distance_vectorized(test_item, train_data, p):
    diff = train_data - test_item
    return np.power(np.sum(np.abs(diff) ** p, axis=1), 1 / p)
...
def main(args: argparse.Namespace) -> float:
    ...
    for test_item in test_data:
        distances = get_distance_vectorized(test_item, train_data, args.p)
        nn_indices = np.argsort(distances)[:args.k]
        nn_classes = train_target[nn_indices]

        if args.weights == "uniform":
            weights = np.ones(args.k)
        
        elif args.weights == "inverse":
            weights = 1 / np.maximum(distances[nn_indices], 1e-5)
        
        elif args.weights == "softmax":
            weights = softmax(-distances[nn_indices])

        res = np.zeros(n_classes)
        
        for cls, weight in zip(nn_classes, weights):
            res[cls] += weight
